...
class MiscSuite01 extends FunSuite with Matchers {
  ...
  test("unit test for SnowflakeTelemetry.getClientConfig") {
    ...
    assert(sparkConfNode.get("spark.driver.extraJavaOptions").asText().contains("-Duser.timezone=GMT"))
    assert(sparkConfNode.get("spark.executor.extraJavaOptions").asText().contains("-Duser.timezone=UTC"))
